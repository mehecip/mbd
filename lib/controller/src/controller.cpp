                         ...
                         const std::string &in_model, std::uint64_t in_idx)
{
  ...
  end_point from{out->second.get(), out_idx, port_dir_t::OUT};
  end_point to{in->second.get(), in_idx, port_dir_t::IN};

  auto [status, conn] = connection::build(from, to);
  if (connection_state::CONNECTED != status)
    return false;
...
bool controller::excution_order()
{
...

  {
    ...
    // set a maximum number of loops to be able to break in case of an
    // arithmetic loop max complexity V * V
...
void controller::update_prio_map(const std::string &comp, prio_map_t &prio_map)
{
  ...
  // only if all ancestors are present in the map, we update the prio of this
  // model to max_prio(ancestors) + 1
  if (mapped_all_ancestors)
    prio_map[comp] = ++prio;
}

// store the final priority in _priority_vect since it will be in the order of
// priorities, so more efficient to iterate over
