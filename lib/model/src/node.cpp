...
connection_state node::connect(std::uint64_t this_out, node* const other, std::uint64_t other_in)
{
	...
	auto other_port = &other->_in_ports[other_in];
...
connection_state node::disconnect(std::uint64_t this_out, node* const other, std::uint64_t other_in)
{
	connection_state state = validate(this_out, other, other_in);
	if (state != connection_state::VALID)
		return state;
...
connection_state node::validate(std::uint64_t this_out, const node* other, std::uint64_t other_in)
{
