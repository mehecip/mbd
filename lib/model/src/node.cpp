...
{
	const connection_state state = validate(this_out, other, other_in);
	...
	_output_connections[this_out].push_back(other_port.get());
...
connection_state node::disconnect(std::uint64_t this_out, const std::unique_ptr<node>& other, std::uint64_t other_in)
{
	...
	inputs.erase(std::remove(inputs.begin(), inputs.end(), other_port.get()), inputs.end());
